...
public:
    ...
    static QString RamrunFname(uint32_t connMode, uint32_t chipId, const std::string& flashModel);
    ...
    int hostPrepareRamRun(bool bWaitVersion = false,bool bWaitMemd=true,bool stopTrace=false);
    int waitMemdMapReady(uint32_t timeout = 10000);
    int waitMmiMapReady(uint32_t timeout = 8000);
    int enableDebugHost();
    int chipRestart();
...
FPLINK_API ChipInfo* CreateChipInfo(TargetConnection* conn);
FPLINK_API void ReleaseChipInfo(ChipInfo* c);
#endif
