...
public:
    ...
    virtual int checkStation(const QString& name, bool& performed, bool& passed) = 0;

    // Set station performed to true, and passed to parameter
    // Errors: CH_STATUS_UNKNOWN_STATION
    virtual int setStationFlag(const QString& name, bool passed) = 0;

    // Clear station flag
    // Errors: CH_STATUS_UNKNOWN_STATION
    virtual int clearStationFlag(const QString &name) = 0;
