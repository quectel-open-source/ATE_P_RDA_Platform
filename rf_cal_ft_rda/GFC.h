...
typedef enum tagEU_NR5GSUB6_BAND_INDEX
{
	...
	NR5GSUB6_N_SIZE = 26	//NR 5G SUB6 频段相关的存储BUFF尺寸（“尺寸”必须比“数量”至少大1, 用于存放结束符）
...
typedef enum tagEU_ANT_TYPE_INDEX
{
	...
	ANT_TYPE_IDX_SIZE = 28,		//“每个天线（所有天线）都有自己的参数存储BUFF”的尺寸（“尺寸”必须比“天线类型的总数量”至少大1, 用于存放结束符）
...
typedef enum tagEU_RF_KEY_TYPE_INDEX
{
	...
	RF_KEY_TYPE_IDX_PXA1802_RF858 = 0,	//ASR 平台 RF Type
	RF_KEY_TYPE_IDX_PAX1908_RF858,		//ASR 平台 RF Type
	RF_KEY_TYPE_IDX_RF866,				//ASR 平台 RF Type
	RF_KEY_TYPE_IDX_RF859C,				//ASR 平台 RF Type
	RF_KEY_TYPE_IDX_CRANE,				//ASR 平台 RF Type
	RF_KEY_TYPE_IDX_CRANE_M,			//ASR 平台 RF Type
	RF_KEY_TYPE_IDX_CRANE_G,			//ASR 平台 RF Type
